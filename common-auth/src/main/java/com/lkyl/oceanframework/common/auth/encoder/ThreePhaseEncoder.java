package com.lkyl.oceanframework.common.auth.encoder;
...
//                throw new CommonException(AuthExceptionEnum.DECODE_ERR);
//            }
//        }
//        return null;
//    }
    ...
    private String getRawCharSeq(String cryptPassword, String privateKey) {
        if(StringUtils.isNotBlank(cryptPassword)){
            try {
                String [] strArr = cryptPassword.split(cryptPassword.substring(32, 61));
                return DesUtil.decryptedDES(strArr[1], strArr[2]);
            } catch (Exception e) {
                log.error("decode error:", e);
                throw new CommonException(AuthExceptionEnum.DECODE_ERR);
            }
